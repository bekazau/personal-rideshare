...
// Width must match the rendered drawer width (Tailwind w-72 = 18rem = 288px).
const DRAWER_WIDTH = 288;
// How far the finger must drag past start-x before we commit to opening on
// release. Below this, the drawer snaps back closed.
const OPEN_THRESHOLD = 80;
// Pixels from the left edge where a touch is allowed to initiate the drag.
const EDGE_ZONE = 25;

export function AppMenu() {
  const [open, setOpen] = useState(false);
  // null = not dragging; number = px the finger has moved right of start.
  ...
  // Finger-tracked swipe handlers — open via left-edge swipe-right when
  // closed, close via swipe-left from anywhere when open. Non-passive on
  // touchmove so preventDefault can block iOS's edge-swipe-back gesture.
  useEffect(() => {
    const onStart = (e: TouchEvent) => {
      if (e.touches.length !== 1) return;
      const x = e.touches[0].clientX;
      // Closed: only start from the left edge zone.
      // Open: allow start anywhere — entire screen is fair game for close.
    ...
    const onMove = (e: TouchEvent) => {
      ...
      // When open, only leftward (negative) movement counts.
      // When closed, only rightward (positive) movement counts.
      const clamped = open
  ...
  // dragX semantics depend on `open`:
  //   open: dragX ∈ [-DRAWER_WIDTH, 0]  (negative = sliding closed)
  //   closed: dragX ∈ [0, DRAWER_WIDTH] (positive = sliding open)
  ...
  return (
    <>
      ...
      {/* Backdrop — always in the DOM so it can fade with the drag. */}
      <div
      ...
      {/* Drawer — always rendered, translated off-screen when closed. */}
      <nav
